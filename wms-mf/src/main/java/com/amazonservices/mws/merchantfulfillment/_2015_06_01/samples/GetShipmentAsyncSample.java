...
public class GetShipmentAsyncSample {
     ...
     * @return The response.
     */
    public static List<Object> invokeGetShipment(
            MWSMerchantFulfillmentServiceAsync client, 
            List<GetShipmentRequest> requestList) {
        // Call the service async.
        List<Future<GetShipmentResponse>> futureList = 
            new ArrayList<Future<GetShipmentResponse>>();
        for (GetShipmentRequest request : requestList) {
            Future<GetShipmentResponse> future = 
                client.getShipmentAsync(request);
            futureList.add(future);
        }
        List<Object> responseList = new ArrayList<Object>();
        for (Future<GetShipmentResponse> future : futureList) {
            Object xresponse;
            try {
                GetShipmentResponse response = future.get();
    ...
    public static void main(String[] args) {
        ...
        // Make the calls.
        GetShipmentAsyncSample.invokeGetShipment(client, requestList);
